...
本节描述 Agent 执行特定任务时应当严格按照该流程执行。

### 部署新应用

- 了解集群现状：阅读 README.md，执行 kubectl
- 梳理问题：阅读待部署的应用的文档、源码、Helm Chart、Kustomization 配置等，梳理清楚配置时需要确认的问题
    - 偏好顺序：官方提供的 Helm Chart/Kustomization 配置 > 第三方配置 > 手动编写配置
- 沟通确认：向人类报告部署计划，提供清晰的背景信息，确认配置细节和变更范围
- 执行变更：按照确认的计划执行变更，等待人类确认变更结果
