...
debian | ubuntu)
    if ! dpkg -l cuda-keyring 2>/dev/null | grep -q ^ii; then
        curl -fsSL "http://developer.download.nvidia.com/compute/cuda/repos/${cuda_repo}/${nvidia_arch}/cuda-keyring_1.1-1_all.deb" \
...
fedora)
    if ! rpm -q cuda-keyring 2>/dev/null; then
        dnf install -y "http://developer.download.nvidia.com/compute/cuda/repos/${cuda_repo}/${nvidia_arch}/cuda-keyring-1.1-1.noarch.rpm" || {
